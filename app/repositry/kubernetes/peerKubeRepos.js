...
class KubernetesPeerRepository extends KubernetesRepository {
    ...
    async createPeerDeployment(data, masterNode) {
        ...
        // const ccDeploymentData = await this.getCCDeployment(data, masterNode);
    ...
    */
            ...
            spec: {
                ...
                template: {
                    ...
                    spec: {
                        ...
                        containers: [
                            {
                                image: `hyperledger/fabric-peer:${config.fabricVersion.peerVersion}`,
                                name: data.peer_enroll_id,
            ...
            spec: {
                ...
                template: {
                    ...
                    spec: {
                        containers: [
                            {
                                image: `couchdb:${config.fabricVersion.couchVersion}`,
                                name: couchName,
                                env: [
                                    {
                                        name: 'COUCHDB_USER',
    ...
    //  * Get deployment configuration for Peer
    //  */
    //  async getCCDeployment(data, masterNode) {
    //     const method = 'getCCDeployment';
    //     logger.debug('%s - start', method);
    //     logger.debug('%s - has received the parameters data: %j', method, data);

    //     const couchName = `${data.name}-couch-${data.orgId.name}`.toLowerCase();
    //     const namespace = data.networkId.namespace;
    //     const CcDeploymentConfig = {
    //         apiVersion: 'apps/v1',
    //         kind: 'Deployment',
    //         metadata: {
    //             name: data.peer_enroll_id + "cc"
    //         },
    //         spec: {
    //             containers: [
    //                 {
    //                     image: `hyperledger/fabric-ccenv:${config.fabricVersion.ccenvVersion}`,
    //                     name: data.peer_enroll_id + "cc",
    //                     env: [
    //                         {
    //                             name: 'FABRIC_LOGGING_SPEC',
    ...
    //     logger.debug('%s - couchName: %s', method, couchName);
    //     logger.debug('%s - namespace: %s', method, namespace);
    //     logger.debug('%s - CcDeploymentConfig: %j', method, CcDeploymentConfig);

    //     return CcDeploymentConfig;
    // }
