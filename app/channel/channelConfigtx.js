...
// =============== fabric 2.x xhanges =======================================
// create channel 
function createChannel(channelBasePath, organisation, orderernode, channelName) {
    ...
    console.log("before running create channel command")
    let status = 'initial'; 
    if (channelName) {
...
// join channel  
function joinChannel(channelBasePath, peers, orderernode, channelName) {
    let status = 'initial';
    ...
    try {
        for (let i = 0; i < channel.organisations.length; i++) {
            ...
            let channelObj = {
                ...
                orderers: {
                    [orderernode.name]: {
                        url: `grpcs://${orderernode.organisation.cluster.masternode.ip}:${orderernode.port}`,
                        grpcOptions: {
                            'ssl-target-name-override': orderernode.name,
                            'grpc.keepalive_timeout_ms': 3000,
                        },
                        tlsCACerts: {
                            path: `${os.homedir}/${config.home}/${orderernode.network.namespace}/${orderernode.organisation.name}-${orderernode.basicca.name}/${orderernode.name}/crypto/msp/tlscacerts/tlsca.pem`,
                        },
                    },
                },
                channels: {
                ...
                certificateAuthorities: {
                    [orgData.ca.name]: {
                        url: `https://${orgData.ca.cluster.masternode.ip}:${orgData.ca.port}`,
                        caName: `${orgData.ca.name}`,
                        tlsCACerts: {
                            path: `${os.homedir}/${config.home}/${orderernode.network.namespace}/${orgData.ca.name}/tls-cert.pem`,
                        },
                        httpOptions: {
                            verify: false,
                        },
            ...
            fs.outputFileSync(`${os.homedir}/${config.home}/channel/${channel.name}/${orgData.name}/config.json`, stringObj);
...
function addChannelInfoToconfig(channel, orderernode) {
    ...
    for (let i = 0; i < channel.organisations.length; i++) {
        let orgData = channel.organisations[i];
        let ccpPath = `${os.homedir}/${config.home}/channel/${channel.name}/${orgData.name}/config.json`;
...
function addPeerinCcp(ccpPath, peerDetails, clusterData, channelName) {
    console.log("in add peer ccp file")
    ...
    console.log("ccp 3rd line")
    let CcpUpdateJson = JSON.stringify(ccp);
    console.log("CcpUpdateJson :",CcpUpdateJson)
    fs.writeFileSync(ccpPath, CcpUpdateJson);
}
// Generates the channelTx for the Channel
// using the shell and the configtxgen binary
