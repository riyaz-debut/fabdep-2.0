...
const coreFabricFilePath = path.join(__dirname, 'core.yaml'); 
console.log("coreFabricFilePath :", coreFabricFilePath)
...
class ChannelController {
    ...
    static async createChannelTx(data) {
        ...
        try {
            ...
            const channelArtifactsPath = `${genesisFilePathbasePath}/channel-artifacts`;
            

            // ==========================================================================================

            // Read the YAML file
            const coreFabricYamlContent = fs.readFileSync(coreFabricFilePath, 'utf8');
            console.log("coreFabricYamlContent :", coreFabricYamlContent)

            // Parse the YAML content into a JavaScript object
            const parsedYaml = yaml.load(coreFabricYamlContent);

            // Convert the parsed object back to YAML string (optional)
            const newYamlString = yaml.dump(parsedYaml);
            
            const copyCoreYamlToChannelArtifactsPath = path.join(channelArtifactsPath, 'core.yaml'); 
            fs.writeFileSync(copyCoreYamlToChannelArtifactsPath, newYamlString || coreFabricYamlContent, 'utf8');

            const copyCoreYamlToWalletPath = path.join(genesisFilePath, 'core.yaml'); // Replace 'config.yaml' with the 
            fs.writeFileSync(copyCoreYamlToWalletPath, newYamlString || coreFabricYamlContent, 'utf8');

            console.log('File copied successfully!');

            // ==========================================================================================

            const channelTxPath = `${os.homedir}/${config.home}/channel/${channelDetail.name}`;

            if (!fs.existsSync(`${os.homedir}/${config.home}/channel`)) {
                fs.mkdirSync(`${os.homedir}/${config.home}/channel`);
            }
    ...
    //create channel on the orderer without any organisation 
     ...
     static async createChannel(data) {
        ...
        try {
            ...
            const result = await configtx.createChannel(basePath, organisation, orderernode, channelDetail.name);
    ...
    static async joinChannel(data) {
        ...
        try {
            ...
            let peers = await peerModel.find({
                _id: { $in: data.peerIds }
              })
              .populate({
                    path: 'orgId',
             ...
             * changes according to fabric version 2.x  
             */
            const joinresult = configtx.joinChannel(basePath, peers, orderernode, channelDetail.name);
