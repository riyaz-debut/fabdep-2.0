...
class PeerController {
    ...
    static async enrollPeer(peerEnrollRequest) {
        ...
        if (peerEnrollRequest.isTLS) {
            ...
            let csr_host3 = "fabdep.com"
            let url = `https://${peer_enroll_id}:${peer_enroll_secret}@${masterNodeIp}:${ca_port}`;
            ...
            let whole_command = `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${peer_msp_path} --enrollment.profile tls --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`;
            ...
            if (
            shell.exec(
                `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${peer_msp_path} --enrollment.profile tls --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`
            ).code !== 0
            ) {
            ...
            let whole_command = `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${peer_msp_path} --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`;
            ...
            if (
            shell.exec(
                `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${peer_msp_path} --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`
            ).code !== 0
            ) {
            shell.echo("Error: while creating genesis block");
            throw new exceptions.EnrollException("Error: while enrolling peer");
    ...
    static async createPeerService(peerKubeRequest) {
        ...
        try {
            ...
            return Promise.resolve({ 
                message: 'peer service created Successfully',
