...
class OrdererController {
    ...
    static async enrollOrderingNodes(ordererRequest) {
        ...
        try {
            ...
            console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@savedOrderingService@@@@@@@@@@@@@@@@@@@", savedOrderingService)
            let clusterData = await cluster.findOne({
                _id: savedOrderingService.clusterId,
            }).populate({
                path: "master_node",
            ...
            let masterNodeIp = clusterData.master_node.ip;
            console.log("masterNodeIp ", masterNodeIp)
            ...
            for (let i = 0; i < orderersList.length; i++) {
                ...
                let csr_host3 = masterNodeIp
                let url = `https://${orderer_enroll_id}:${orderer_enroll_secret}@${masterNodeIp}:${tls_ca_port}`;

                const path = `${os.homedir}/fabric-samples/bin:$PATH`;
                let whole_command = `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_tls_path} --enrollment.profile tls --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`;

                //Raft
                if (
                    shell.exec(
                    `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_tls_path} --enrollment.profile tls --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`
                    ).code !== 0
                ) {
                ...
                let whole_command = `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_msp_path} --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`;

                //Raft
                if (
                    shell.exec(
                    `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_msp_path} --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --csr.hosts ${csr_host3} --tls.certfiles ${tlsPath}`
                    ).code !== 0
                ) {
                    shell.echo("Error: while creating genesis block");
