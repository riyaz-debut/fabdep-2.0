...
function getOrganisations(organisations) {
  ...
  function getGenesisRaftTx(orderingList, ordererDetail) {
    ...
    for (let i = 0; i < orderingList.length; i++) {
      ...
      console.log("basePath ", basePath); //home/developer/${config.home}/network1/orderer-test-ca-root-test
	...
	return {
		Profiles: {
			Solo: {
				...
				Orderer: {
					OrdererType: "solo",
					Addresses: [`${orderer.name}:${orderer.port}`],
					BatchTimeout: "2s",
					BatchSize: {
						MaxMessageCount: 10,
						AbsoluteMaxBytes: "99 MB",
...
function generateOrgConfig(ConfigtxPath) {
	...
	if (
		shell.exec(
			`export FABRIC_CFG_PATH=${ConfigtxPath}/org1new-artifacts   && configtxgen -printOrg Org1MSP > ${ConfigtxPath}/org1new-artifacts/org1.json`
		).code !== 0
	) {
