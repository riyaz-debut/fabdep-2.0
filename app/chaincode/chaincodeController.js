...
class ChaincodeController {
  ...
  static async uploadChaincode(data, chaincodeFile) {
    ...
    try {
      ...
      if (zip.isZipSync(chaincodeFile.path)) {
        ...
        logger.debug("%s - sourcefolderPath: %s", method, sourcefolderPath);
        ...
        if (data.type == config.GOLANG) {
          ...
          mainpaths = await shell
            ...
            .split("\n")
            .filter(Boolean);
          logger.debug("%s - mainpaths: %s", method, mainpaths);
          if (!mainpaths.length) {
  ...
  static async installChaincode(data) {
    ...
    try {
      ...
      console.log("===========channelId data =====================", data.channelId)
      ...
      // chaincode lifecycle processes(package, install, approve, commit readiness, commit)
      ...
      if (packageCCResult.status === 'SUCCESS') {
        logger.debug('%s - package chaincode success %j', method, packageCCResult);
        ...
        if (installCCResult.status === 'SUCCESS') {
          logger.debug('%s - install chaincode success %j', method, installCCResult);
          ...
          if (queryInstallCCResult.status === 'SUCCESS') {
            logger.debug('%s - query chaincode success %j', method, queryInstallCCResult);
            ...
            if (approveCCResult.status === 'SUCCESS') {
              logger.debug('%s - approve chaincode success %j', method, approveCCResult);
              ...
              if (checkCommitCCResult.status === 'SUCCESS') {
                logger.debug('%s - check commit  chaincode success %j', method, checkCommitCCResult);
                ...
                if (commitChaincodeResult.status === 'SUCCESS') {
                  logger.debug('%s - commit chaincode success %j', method, commitChaincodeResult);
   ...
   * @param {*} data
   */
   static async invokeQueryChaincode(data) {
    console.log("in invokeQueryChaincode Fx")
    const method = "invokeQueryChaincode";
    logger.debug("%s - start", method);
    logger.debug("%s - has received the parameters %j", method, data);
    ...
    try {
      ...
      console.log("before channel data fetch")
      // channel and orderer details
      let channelDetailArray = await channelConfigtx.fetchChannelPrereqsiteData(data);
      console.log("after channel data fetch")
      if (!channelDetailArray.length) {
        throw new Exception.ChannelNotFound();
      }
      let channelDetail = channelDetailArray[0];
      ...
      let invokeCCResult = await connectionFileConfig.invokeChaincode(peer, orderernode, chaincode, channelDetail.name)
      console.log("invokeCCResult=============================", invokeCCResult)

      // let queryCCResult = await connectionFileConfig.queryChaincode(peer, orderernode, chaincode, channelDetail.name)
      // console.log("queryCCResult=============================", queryCCResult)

      message = "chaincode invoke successfully";
      return {
        status: 200,
        data: {
          message: message
