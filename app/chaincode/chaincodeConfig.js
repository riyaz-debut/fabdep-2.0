...
function packageChaincode(peer, orderernode, chaincode, ccPath) {
    ...
    const CC_SRC_PATH = `${ccPath}/${CC_NAME}`
...
function invokeChaincode(peer, orderernode, chaincode, channelName) {
    const orgPath = utils.getBasePath(peer.networkId.namespace, peer.orgId.name, peer.caId.name);
    const peerOrgPath = `${orgPath}/${peer.peer_enroll_id}`;
    const ordererOrgPath = utils.getBasePath(peer.networkId.namespace, orderernode.orgId.name, orderernode.caId.name);
    const ordererNodePath = `${ordererOrgPath}/${orderernode.name}`;
    ...
    let status = 'initial'; 
    if (chaincode) {
        if (shell.exec(` set -x export PATH=${path} && export FABRIC_CFG_PATH=${fabric_cfg_path} && export CORE_PEER_TLS_ENABLED=${core_peer_tls_enabled} && export CORE_PEER_LOCALMSPID=${core_peer_localmspid} && export CORE_PEER_TLS_ROOTCERT_FILE=${core_peer_tls_rootcert_file} && export CORE_PEER_MSPCONFIGPATH=${core_peer_mspconfigpath} && export CORE_PEER_ADDRESS=${core_peer_address} && peer chaincode invoke -o ${ordererAddress} --tls --cafile ${ordererCA} -C ${channelName} -n ${CC_NAME} --peerAddresses ${core_peer_address} --tlsRootCertFiles ${core_peer_tls_rootcert_file} -c '{"function":"InitLedger","Args":[]}'`).code !== 0) {
            shell.echo('Error: while invoke chaincode');
            throw new exceptions.ChaincodeException('Error: while invoke chaincode');
        }
        status = 'SUCCESS'
    } 
    return ({status : status})
...
// Query chaincode 
function queryChaincode(peer, orderernode, chaincode, channelName) {
    const orgPath = utils.getBasePath(peer.networkId.namespace, peer.orgId.name, peer.caId.name);
    const peerOrgPath = `${orgPath}/${peer.peer_enroll_id}`;
    const ordererOrgPath = utils.getBasePath(peer.networkId.namespace, orderernode.orgId.name, orderernode.caId.name);
    const orgMsp = peer.orgId.mspId
    ...
    // peer chaincode query -C ${channelName} -n ${CC_NAME} -c '{"Args":["readMarble","marble1"]}'
    // peer chaincode query -C myc1 -n marbles -c '{"Args":["queryMarblesByOwner","tom"]}'
    let status = 'initial'; 
    if (chaincode) {
        if (shell.exec(` set -x export PATH=${path} && export FABRIC_CFG_PATH=${fabric_cfg_path} && export CORE_PEER_TLS_ENABLED=${core_peer_tls_enabled} && export CORE_PEER_LOCALMSPID=${core_peer_localmspid} && export CORE_PEER_TLS_ROOTCERT_FILE=${core_peer_tls_rootcert_file} && export CORE_PEER_MSPCONFIGPATH=${core_peer_mspconfigpath} && export CORE_PEER_ADDRESS=${core_peer_address} && peer chaincode query -C ${channelName} -n ${CC_NAME} -c '{"Args":["GetAllAssets"]}'`).code !== 0) {
            shell.echo('Error: while query chaincode');
            throw new exceptions.ChaincodeException('Error: while query chaincode');
        }
        status = 'SUCCESS'
    } 
    return ({status : status})
...
function getPeersObject(peers) {
    ...
    peers.forEach(peerData => {
        peerObj[peerData.peer_enroll_id] = {
            url: `grpcs://${peerData.orgId.clusterId.master_node.ip}:${peerData.peerport}`,
            tlsCACerts: {
                path: `${os.homedir}/${config.home}/${peerData.networkId.namespace}/${peerData.orgId.name}-${peerData.caId.name}/msp/tlscacerts/tlsca.pem`,
            },
            grpcOptions: {
